...
namespace rigid2d
{
  ...
  public:
    ...
    // TODO: pass in wheel base and radius to DiffDrive
    ...
    /// \param pose - pose of robot in world coordinates
    double waypointDistance(Pose pose);

    /// \brief compose difference in heading of robot to waypoint
    /// \param pose - pose of robot in world coordinates
    /// \return difference in heading
    double waypointHeading(Pose pose);

    /// \brief compose control solution for driving between
    ///         the waypoints
    void trajectory();
